...
HEADM = (Matrix.Translation((0, 0, head_cz))
         @ Matrix.Rotation(math.radians(HEAD_YAW), 4, 'Z')
...
SH_L = _mx(SH_L); HAND_L = _mx(HAND_L); FIST_C = _mx(FIST_C); GRIP = _mx(GRIP)
BOW_AX = _mx(BOW_AX); Z_FIST = _mx(Z_FIST)
tip_top = _mx(tip_top); tip_bot = _mx(tip_bot)
ARM_CHAIN = [(_mx(p), r) for p, r in ARM_CHAIN]
bow_pts = [(_mx(p), r) for p, r in bow_pts]
S_SEGS = [(_mx(a), _mx(b)) for a, b in S_SEGS]
ROT_L = _MM @ ROT_L
HAND_R_END = _mx(Vector(HAND_R_END))
if R_HAND_MODE == "open":
    RotR = _MM @ RotR
else:
    ROT_R = _MM @ ROT_R
    NOCK = _mx(NOCK); APEX = _mx(APEX); ARROW_BACK = _mx(ARROW_BACK)
    arrow_end = _mx(arrow_end)
...
# mittellinie (x=0). Haende nahe der Mittellinie (|x| < 0.2, z. B. Zughand
# am Nock) sind mehrdeutig und werden nur geloggt.
...
for _hn, _hc, _hr in _hands:
    ...
    _ok = (-_hc.x) * _thumb.x            # >0 = Daumen zeigt Richtung Mitte
    _amb = abs(_hc.x) < 0.2
    print(f"HAND-CHIRALITAET {_hn}: Hand-x {_hc.x:+.3f} Daumen-x {_thumb.x:+.3f} "
...
# SPIELKAMERA (el 60, az 45 — verbindliche Konvention v07) nicht verdecken
if POSE == "attack":
    el, az = math.radians(60), math.radians(45)
    ...
    print(f"GESICHT-FREI (Kamera-Projektion): Zughand {d_face_hand:.3f} "
          f"(>= 0.05) | rechter Arm {d_face_arm:.3f} (>= 0.035)")
    assert d_face_hand >= 0.05, f"Zughand verdeckt das Gesicht ({d_face_hand:.3f})"
    assert d_face_arm >= 0.035, f"Zugarm zu nah am Gesicht ({d_face_arm:.3f})"
...
ko.rotation_euler = (math.radians(50), math.radians(12), math.radians(35))
...
el, az, d = math.radians(60), math.radians(45), 24.0   # az45 = Gebaeude-Konvention
...
cam_data.ortho_scale = 1.13 * max(max(cu) - min(cu), max(cw) - min(cw))
cam.location = target + Cdir * d
cam.rotation_euler = (target - cam.location).to_track_quat("-Z", "Y").to_euler()

# FUSSPUNKT-PROJEKTION (arg "footproj"): projiziert die beiden Stiefel-Sohlen
# durch die Render-Kamera und druckt ihre Render-Pixel (fuer die exakte
# Anker-Bestimmung des Export-Masters). Rendert/schreibt NICHT.
if "footproj" in argv:
    ...
    for nm in ("stiefel_sohle_L", "stiefel_sohle_R"):
        w = _sole_contact(nm)
        uv = world_to_camera_view(sc, cam, w)
        print(f"FOOTPROJ {nm} world {tuple(round(c, 3) for c in w)} "
              f"render_px {uv.x * sc.render.resolution_x:.2f} "
              f"{(1 - uv.y) * sc.render.resolution_y:.2f}")
    import sys as _sysx
    _sysx.exit(0)

# SCHREIBSCHUTZ (Nutzer v06): bestehende Dateien werden NIE ueberschrieben,
# sondern brechen mit Fehlermeldung ab (v-Nummer erhoehen statt clobbern).
_out = os.path.join(OUT, f"menschen_archer_{POSE}_v07.png")
if os.path.exists(_out):
    raise SystemExit(f"ABBRUCH Schreibschutz: {_out} existiert bereits — "
                     f"v-Nummer erhoehen statt ueberschreiben.")
